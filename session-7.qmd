...
We will now explore count models, which are relevant in a range of transport applications for modelling variables such as the number of trips, number of vehicles, or number of collisions.
...
::: {.callout-tip title="Does the variance approximate the mean?"}
...
> The incidence rate of active travel trips is 2.3 times higher for families with one or more children than for those without any.
...
One of the key assumptions of the Poisson model is that the mean is approximately equal to the variance. Dispersion can occur when the data is more variable than the model assumes. This can create estimation problems, such as underestimated errors (e.g. we can conclude something is significant when it is not), or poor model fit.
...
A common alternative is to use a negative binomial model. This is an extension of the standard Poisson model and does not assume that the variance is roughly similar to the mean of our count. To estimate this model, we will require the `glm.nb()` function of the `MASS::` package.
